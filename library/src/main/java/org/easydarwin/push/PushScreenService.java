...
//        liveData.postValue(new MediaStream.PushingState(0, "未开始", true));
        mPushThread = new Thread(){
            ...
            @Override
            public void run() {
                Notification.Builder builder = new Notification.Builder(PushScreenService.this)
                        .setContentTitle(getString(R.string.screen_pushing))
                        .setSmallIcon(R.drawable.ic_pusher_screen_pushing)
                        .addAction(new Notification.Action(R.drawable.ic_close_pushing_screen,"关闭",PendingIntent.getBroadcast(getApplicationContext(),10000,new Intent(ACTION_CLOSE_PUSHING_SCREEN),FLAG_CANCEL_CURRENT)));
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    String channel_id = getPackageName();
                    String channel_name = "AnPushScreen";
